...
static void Pin_temper1_Write(uint8 u8Data)
{
	if(0 == u8Data)
	{
		// GPIO_WriteBit(TEMPER_GPIO_PORT, TEMPER1_GPIO_PIN, 0);
		TEMPER_GPIO_PORT->ODR |= TEMPER1_GPIO_PIN;
	}
	else
	{
		// GPIO_WriteBit(TEMPER_GPIO_PORT, TEMPER1_GPIO_PIN, 1);
		TEMPER_GPIO_PORT->ODR &= (uint8_t)(~TEMPER1_GPIO_PIN);
	}
}
...
static void Pin_temper2_Write(uint8 u8Data)
{
	if(0 == u8Data)
	{
		...
		TEMPER_GPIO_PORT->ODR |= TEMPER2_GPIO_PIN;
	}
	else
	{
		// GPIO_WriteBit(TEMPER_GPIO_PORT, TEMPER2_GPIO_PIN, 1);
		TEMPER_GPIO_PORT->ODR &= (uint8_t)(~TEMPER2_GPIO_PIN);
	}
}
...
{
	// if(0 == GPIO_ReadInputDataBit(TEMPER_GPIO_PORT, TEMPER1_GPIO_PIN))
	if(0 == (TEMPER_GPIO_PORT->IDR & (uint8_t)TEMPER1_GPIO_PIN))
	{
		return 0;         //
	}
	else
...
static uint8 Pin_temper2_Read(void)
{
	...
	if(0 == (TEMPER_GPIO_PORT->IDR & (uint8_t)TEMPER2_GPIO_PIN))
	{
		return 0;         //
	}
	else
...
static sint32 Ds18B20ReadAll(void)
{
	...
	for(i = 0; i < TEMPRATURE_MAX_NUM; i ++)
	{
		...
		if((0 == s32Ret)&& 
		   (temper_85_err_check(tTemperRead.s16Temp, tTempSampleCtrl.tTemperData[i].s16Temp)))
		{
			// 温度滤波
			tTemperRead.s16Temp = TemperFilter(tTempSampleCtrl.tTemperData[i].s16TempBuf, tTemperRead.s16Temp, 
											   tTempSampleCtrl.tTemperData[i].s16Temp, 
											   tTempSampleCtrl.u8Ds18b20Status[i]);
			
			tTempSampleCtrl.tTemperData[i].s16Temp = tTemperRead.s16Temp;
		...
		else if(0 != s32Ret)
		{
			...
			{
				tTempSampleCtrl.tTemperData[i].s16Temp= TEMP_ERROR_VALUE;
...
{	
	...
	{
		tTempSampleCtrl.u8TempSampleStatus = DS18B20_STATUS_IDLE;
		tTempSampleCtrl.u8SampleFlag = 1;
	}
...
sint8 TemperTaskInit(void)
{
	...
	GPIO_Init(TEMPER_GPIO_PORT, TEMPER_POWER_GPIO_PIN, GPIO_Mode_Out_PP_High_Slow);
