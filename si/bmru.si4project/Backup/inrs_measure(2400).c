...
static sint8 InrsMesureAct(void)
{
	...
	uint32 u32InRs[FRAME2_KPULSE_NUM / 2]; // 内阻值[放电脉冲次数]
	...
	for(i = 0; i < BATTERY_MAX_NUM; i ++)
	{
		...
		{
			u32InRs[j] = (((uint32)(u16AdData[i][0][1][j] - u16AdData[i][1][1][j]) * INRS_VREF_RS_12V) / \
								(uint32)(u16AdData[i][0][0][j] - u16AdData[i][1][0][j]));

			#if DEBUG_PRINTF
			// u32InRs[j] =99.99;
			sprintf((char *)DebugBuf, "bat%u+:%u,%u,%lu\r\n", (uint16)(i+1), u16AdData[i][0][1][j], u16AdData[i][0][0][j], u32InRs[j]);
			DebugStrOutput(DEBUG_LEVEL_INFO, DebugBuf);
			
			sprintf((char *)DebugBuf, "bat%u-:%u,%u,%lu\r\n", (uint16)(i+1), u16AdData[i][1][1][j], u16AdData[i][1][0][j], u32InRs[j]);
			DebugStrOutput(DEBUG_LEVEL_INFO, DebugBuf);
			#endif
		}
