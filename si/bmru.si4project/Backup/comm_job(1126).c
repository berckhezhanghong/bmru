...
static sint16 RegCheck(uint8 u8Cmd, uint16 u16Reg, uint16 u16Num)
{
...
static sint16 AppDeal(tProtocolInfoDef tProtocolInfo, tSendProtocolInfoDef *ptSendProtocolInfo)
{
	...
	switch(tProtocolInfo->u8CommandCode)
	{
		case CMD_TYPE_READ:
		{
			pu8Addr = RegCheck(ptSendProtocolInfo.u8CommandCode, ptSendProtocolInfo.u16RegisterStartAddr, ptSendProtocolInfo.u16RegisterNum)
			if(NULL != pu8Addr)
			{
				memcpy(ptSendProtocolInfo->pu8Data, pu8Addr, (ptSendProtocolInfo.u16RegisterNum * 2)
			}
			else
			{
				s16Ret = -1;
