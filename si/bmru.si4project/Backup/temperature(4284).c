...
static void Pin_temper1_Write(uint8 u8Data)
{
	if(0 == u8Data)
	{
		HT_GPIOA->PTDAT &= ~0x1000;         //PA12  output 0;
	}
	else
	{
		HT_GPIOA->PTDAT |= 0x1000;         //PA12  output 1;
	}
}
...
static void Pin_temper2_Write(uint8 u8Data)
{
	if(0 == u8Data)
	{
		HT_GPIOA->PTDAT &= ~0x2000;         //PA13  output 0;
	}
	else
	{
		HT_GPIOA->PTDAT |= 0x2000;         //PA13  output 1;
	}
}
...
{
	if(0 == (HT_GPIOA->PTDAT & 0x1000))
	{
		return 0;         //PA12  output 0;
	}
	else
	{
		return 1;         //PA12  output 1;
	}
}
...
{
	if(0 == (HT_GPIOA->PTDAT & 0x2000))
	{
		return 0;         //PA13  output 0;
	}
	else
	{
		return 1;         //PA13  output 1;
	}
}
...
static void TemperPowerCtrl(uint8 u8Type)
{
	if(0 == u8Type)
	{
		HT_GPIOD->PTDAT |= 0x0080;         //PD7  output 0;
	}
	else
	{
		HT_GPIOD->PTDAT &= ~0x0080;         //PD7  output 1;
	}
}
...
static sint32 OneWireByteWrite(uint8 u8Channal, uint8 u8Status)
{
	...
	if(0 == u8Channal)
	{
		HT_GPIOA->PTDIR |= 0x1000;		   //PA12 as output
	}
	else if(1 == u8Channal)
	{
		HT_GPIOA->PTDIR |= 0x2000;		   //PA13 as output
	}
...
static uint8 OneWireByteRead(uint8 u8Channal)
{
	...
	if(0 == u8Channal)
	{
		for (i = 0; i < 8; i ++)
		{
			...
			HT_GPIOA->PTDIR &= ~0x1000;		   //PA12 as input
	...
	else if(1 == u8Channal)
	{
		for (i = 0; i < 8; i ++)
		{
			...
			HT_GPIOA->PTDIR &= ~0x2000;		   //PA13 in input
...
tTempSampleCtrlDef *get_temper_info(void)
{
	return &tTempSampleCtrl;
}


sint8 temper_task_init(void)
{
	EnWr_WPREG();
	HT_GPIOA->IOCFG &= ~0x3000;         //PA12,13 as GPIO
	DisWr_WPREG();
	HT_GPIOA->PTDIR |= 0x3000;		   	//PA12,13 as output

	EnWr_WPREG();
	HT_GPIOD->IOCFG &= ~0x0080;         //PD7 as GPIO
	DisWr_WPREG();
	HT_GPIOD->PTDIR |= 0x0080;		   	//PD7 as output
