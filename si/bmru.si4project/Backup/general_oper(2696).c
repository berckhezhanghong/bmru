...
uint16 AdcGeneralAcq(uint8 u8AdcChan)
{
	...
	fVoltData = u16AdcData;
	
	sprintf(DebugBuf, "111 = %u %f\r\n", u16AdcData, fVoltData);
	DebugStrOutput(DEBUG_LEVEL_INFO, DebugBuf);
	
    fVoltData = (fVoltData * 1225.0) / u16ReferAdcValue;

    sprintf(DebugBuf, "112 = %f\r\n", fVoltData);
	DebugStrOutput(DEBUG_LEVEL_INFO, DebugBuf);

	  sprintf(DebugBuf, "113 = %f\r\n", (uint16)(fVoltData));
	DebugStrOutput(DEBUG_LEVEL_INFO, DebugBuf);
    
    return (uint16)(fVoltData);
}
...
void AdcCaliInit(void)
{
	...
	{
		// 
		u16VoltAd[i] = AdcGeneralAcq(SETX_ADCIN_REF1V2);
	}
